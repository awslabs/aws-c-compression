...
static uint8_t encoded_url[] = { 0x9e, 0x79, 0xeb, 0x9b, 0x04, 0xb3, 0x5a, 0x94, 0xd5, 0xe0, 0x4c, 0xdf, };
...
static int test_huffman_symbol_encoder(struct aws_allocator *allocator, void *user_data) {
...
static int test_huffman_encoder(struct aws_allocator *allocator, void *user_data) {
    ...
    aws_common_error result = aws_huffman_encode(&encoder, url_string, &processed, output_buffer, &output_size);
...
static int test_huffman_encoder_all_code_points(struct aws_allocator *allocator, void *user_data) {
    ...
    aws_common_error result = aws_huffman_encode(&encoder, all_codes, &processed, output_buffer, &output_size);
    ASSERT_SUCCESS(result);
...
static int test_huffman_encoder_partial_output(struct aws_allocator *allocator, void *user_data) {
    ...
    for (int i = 0; i < sizeof(step_sizes) / sizeof(size_t); ++i) {
        ...
        for (size_t bytes_to_write = encoded_codes_len; bytes_to_write > 0; ) {
            ...
            const char *current_input = all_codes + all_codes_len - bytes_to_read;
            uint8_t *current_output = output_buffer + bytes_written;
            size_t bytes_read = bytes_to_read;

            aws_common_error result = aws_huffman_encode(&encoder, current_input, &bytes_read, current_output, &output_size);
...
static int test_huffman_symbol_decoder(struct aws_allocator *allocator, void *user_data) {
...
static int test_huffman_decoder(struct aws_allocator *allocator, void *user_data) {
    ...
    aws_common_error result = aws_huffman_decode(&decoder, encoded_url, &processed, output_buffer, &output_size);
    ASSERT_SUCCESS(result);
...
static int test_huffman_decoder_all_code_points(struct aws_allocator *allocator, void *user_data) {
    ...
    aws_common_error result = aws_huffman_decode(&decoder, encoded_codes, &processed, output_buffer, &output_size);
    ASSERT_SUCCESS(result);
...
static int test_huffman_decoder_partial_input(struct aws_allocator *allocator, void *user_data) {
    ...
    for (int i = 0; i < sizeof(step_sizes) / sizeof(size_t); ++i) {
        ...
        for (size_t bytes_to_process = encoded_codes_len; bytes_to_process > 0; ) {
            ...
            aws_common_error result = aws_huffman_decode(&decoder, current_input, &to_process, current_output, &output_size);
...
static int test_huffman_decoder_partial_output(struct aws_allocator *allocator, void *user_data) {
    ...
    for (int i = 0; i < sizeof(step_sizes) / sizeof(size_t); ++i) {
        ...
        for (size_t bytes_to_process = encoded_codes_len; bytes_written < all_codes_len; ) {
            ...
            aws_common_error result = aws_huffman_decode(&decoder, current_input, &processed, current_output, &output_size);
            ...
            if (bytes_written == all_codes_len) {
                ASSERT_SUCCESS(result);
            }
        }
...
static int test_huffman_transitive(struct aws_allocator *allocator, void *user_data) {
    ...
    size_t processed = url_string_len;
    aws_common_error result = aws_huffman_encode(&encoder, url_string, &processed, intermediate_buffer, &encode_output_size);
    ...
    result = aws_huffman_decode(&decoder, intermediate_buffer, &processed, output_string, &decode_output_size);
...
static int test_huffman_transitive_all_code_points(struct aws_allocator *allocator, void *user_data) {
    ...
    size_t encode_output_size = sizeof(intermediate_buffer);
    size_t processed = all_codes_len;
    aws_common_error result = aws_huffman_encode(&encoder, all_codes, &processed, intermediate_buffer, &encode_output_size);
    ...
    processed = encode_output_size;
    result = aws_huffman_decode(&decoder, intermediate_buffer, &processed, output_string, &output_size);
