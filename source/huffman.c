...
static uint8_t MAX_PATTERN_BITS =
    BITSIZEOF(((struct aws_huffman_code *)0)->pattern);
...
static int encode_write_bit_pattern(
    ...
    struct aws_huffman_code bit_pattern) {
    ...
    while (bits_to_write > 0) {
        ...
        state->working |= (bit_pattern.pattern << bits_to_cut) >>
                          (MAX_PATTERN_BITS - state->bit_pos);
            ...
            if (state->output_cursor.len == 0) {
                ...
                if (bits_to_write) {
                    state->encoder->overflow_bits.pattern =
                        (bit_pattern.pattern << bits_to_cut) >>
                        (MAX_PATTERN_BITS - bits_to_write);
                }
...
int aws_huffman_encode(
    ...
    struct aws_byte_cursor input_cursor =
        aws_byte_cursor_from_array(to_encode, *length);
    ...
    while (input_cursor.len) {
        ...
        struct aws_huffman_code code_point =
            encoder->coder->encode(new_byte, encoder->coder->userdata);
...
static void decode_fill_working_bits(struct decoder_state *state) {
    ...
    while (state->decoder->num_bits < MAX_PATTERN_BITS &&
        ...
        uint64_t positioned = ((uint64_t)new_byte)
                              << (BITSIZEOF(state->decoder->working_bits) - 8 -
                                  state->decoder->num_bits);
...
int aws_huffman_decode(
    ...
    struct aws_byte_cursor output_cursor =
        aws_byte_cursor_from_array(output, *output_size);
    ...
    while (1) {
        ...
        uint8_t bits_read = decoder->coder->decode(
            (uint32_t)(
                decoder->working_bits >>
                (BITSIZEOF(decoder->working_bits) - MAX_PATTERN_BITS)),
            &symbol,
