...
"static struct aws_huffman_code code_points[] = {\n");
    ...
    for (size_t code_point_idx = 0; code_point_idx < num_code_points; ++code_point_idx) {
        struct huffman_code_point *cp = &code_points[code_point_idx];
        fprintf(file,
"    { .pattern = 0x%x, .num_bits = %u }, /* '%c' %u */\n",
            cp->code.bits, cp->code.num_bits, isprint(cp->symbol) ? cp->symbol : ' ', cp->symbol);
    }
...
"}\n"
"\n"
"struct aws_huffman_symbol_coder *%s_get_coder() {\n"
"\n"
